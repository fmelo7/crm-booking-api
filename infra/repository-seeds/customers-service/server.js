...
const SERVICE_NAME = 'customers-service';
process.env.SERVICE_NAME = SERVICE_NAME;
const PORT = process.env.CUSTOMERS_SERVICE_PORT || process.env.PORT || 3002;
...
log('info', 'Customers service database provider resolved', {
...
if (isEnvDebugEnabled()) {
  log('info', 'Customers service environment variables loaded', {
    env: getMaskedEnv(),
  });
}
...
const connectDatabaseWithRetry = async () => {
  ...
  for (let attempt = 1; attempt <= DATABASE_CONNECT_RETRIES; attempt += 1) {
    try {
      ...
      lastError = err;
      log('warn', 'Customers service database connection attempt failed', {
        databaseProvider: DATABASE_PROVIDER,
        attempt,
...
const bootstrap = async () => {
  ...
  try {
    ...
    log('info', 'Customers service database connected', {
      databaseProvider: DATABASE_PROVIDER,
      databaseUri: maskDatabaseUri(uri),
    });
  } catch (err) {
    healthState.setDatabaseConnected(false);
    log('error', 'Customers service database connection failed', {
      databaseProvider: DATABASE_PROVIDER,
      error: {
        message: err.message,
        stack: err.stack,
  ...
  log('info', 'Customers service HTTP server started', {
    port: Number(PORT),
    runtime: 'nestjs',
...
bootstrap()
  .catch((err) => {
    log('error', 'Customers service HTTP server failed to start', {
      error: {
        message: err.message,
        stack: err.stack,
      },
