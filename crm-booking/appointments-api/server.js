...
const SERVICE_NAME = 'appointments-api';
process.env.SERVICE_NAME = SERVICE_NAME;
const PORT = process.env.APPOINTMENTS_API_PORT || process.env.PORT || 3001;
...
log('info', 'Appointments service database provider resolved', {
...
if (isEnvDebugEnabled()) {
  log('info', 'Appointments service environment variables loaded', {
    env: getMaskedEnv(),
  });
}
...
const connectDatabaseWithRetry = async () => {
  ...
  for (let attempt = 1; attempt <= DATABASE_CONNECT_RETRIES; attempt += 1) {
    try {
      ...
      lastError = err;
      log('warn', 'Appointments service database connection attempt failed', {
        databaseProvider: DATABASE_PROVIDER,
        attempt,
...
const bootstrap = async () => {
  ...
  log('info', 'Appointments service HTTP server started', {
    port: Number(PORT),
    runtime: 'nestjs',
...
bootstrap()
  .catch((err) => {
    log('error', 'Appointments service HTTP server failed to start', {
      error: {
        message: err.message,
        stack: err.stack,
      },
